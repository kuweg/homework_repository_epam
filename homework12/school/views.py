...
def set_homework(request):
    ...
    create_homework(homework_task=hometask,
                    deadline_days=deadline,
                    author_id=teacher_id)

    return JsonResponse({'success': True,
                         'object': Homework.objects.get(text=hometask).text})
...
def check_homework_for_being_valid(request):
    ...
    return JsonResponse({'success': True,
                         'acccept': HomeworkResult.objects.get(
                             pk=homework_res_id
                             ).is_accepted}
                        )
...
def complete_homework(request):
    ...
    do_homework(homework_id=homework_id,
                solution=solution,
                author_id=student_id)
    return JsonResponse({'Success': True,
                         'Solution': HomeworkResult.objects.get(
...
def reset_results(request):
    ...
    reset_homework_results(homework_id)
    return JsonResponse({'Success': True})
